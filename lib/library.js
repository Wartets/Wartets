...
function initLibrary() {
	...
	function renderDocuments() {
		...
		const searchQuery = searchInput ? searchInput.value.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : '';
		const highlightRegex = searchQuery ? new RegExp(`(${searchQuery})`, 'gi') : null;
		...
		processedDocs.forEach(doc => {
			...
			if (card.dataset.viewMode !== currentViewMode || searchQuery !== card.dataset.lastQuery) {
	...
	function applyFilters() {
		...
		cards.forEach(card => {
			...
			if (!matchesCategory && card.dataset.categoryIds) {
				const ids = JSON.parse(card.dataset.categoryIds);
				matchesCategory = ids.includes(parseInt(currentFilter)) || ids.includes(currentFilter);
			}
		...
		if (state && typeof Flip !== 'undefined') {
			Flip.from(state, {
				...
				onLeave: elements => gsap.to(elements, 
					{ opacity: 0, duration: 0.2, onComplete: () => elements.forEach(el => el.style.display = 'none') }
				)
			});
		}
